...
子どもが「光が水に入ると曲がるのって、虹もそういうことなの？」と聞いてきたとき、授業の進度があって「それは今日はやらないよ」と流してしまうことがあるんです。子どもの目が一瞬キラッとしたのに、もったいないなと。
...
チャット画面はシンプルな LINE 風 UI です。テキスト入力でもマイクでも話しかけられます。
...
顔認識カメラをオンにすると、困り顔・悲しそうな表情が続いたとき先生が自動で「難しかった？もっとわかりやすく説明しようか？」と声をかけます。ブラウザ完結（カメラ映像はサーバーに送らない）なのでプライバシーも安心です。
...
子どもの発話
     ...
     ▼
┌─────────────────────┐
│   Generator Agent   │  ← 「面白さ」を最大化
│  temperature: 0.9   │    横道候補を即時生成・子どもへ先に返答
└─────────┬───────────┘
          │ branchCandidate
          ▼
┌─────────────────────┐
│   Evidence Tool     │  ← 根拠検索（決定的処理）
│  prefectures_       │    キーワードで JSON を引く
│  sources.json       │
└─────────┬───────────┘
          │ evidenceEntries
          ▼
┌─────────────────────┐
│    Judge Agent      │  ← 「正確さ」を最大化
│  temperature: 0.3   │    事実性・学年適合・安全性を審査
└─────────┬───────────┘
          │
     ┌────┴────┐
     │         │
...
看護師がバイブコーディングで作ったにしては、われながら欲張りな構成になりました。
