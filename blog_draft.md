...
キッズプログラミングの教室に関わっているのですが、ふとした瞬間に気になることがありました。
...
最初は別画面で選ばせる設計にしていたのですが、「フォームっぽくてつまらない」のと、何を聞いたらわからない子もいるから選択のほうが学習意欲を削がないのではないかと気づいて変えました。先生が口頭で聞いてくれる方が、アプリとして自然だし会話っぽくて良いなと。

### 子どもの「横道」を逃さない

<!-- スクリーンショット：チャット画面 LINE風UI -->

チャット画面はシンプルな LINE 風 UI です。テキスト入力でもマイクでも話しかけられます。スマホ・タブレット対応のレスポンシブデザインなので、子どもが普段使っているデバイスでそのままブラウザから使えますし、わからないことが出てきた時もわざわざパソコンを開かなくてもいいので手軽です。またアプリのインストールも不要です。
...
![ブランチラーニング アーキテクチャ図](https://static.zenn.studio/user-upload/8d9a09f5a38d-20260528.png)
...
コードを書くのは Claude に頼りっぱなしですが、「何を作りたいか」「なぜそれが必要か」だけは自分の中にしっかりありました。バイブコーディングってそういうものかもしれません。
